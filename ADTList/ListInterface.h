...
@pre none
@post Entries in the list will now appear in reverse order
@param none
@return void */
  virtual void reverse() = 0;

/** Replaces an entry at a given position in the list with a new provided entry
@pre 1 <= position <= getLength().
@post The entry at the given position is now newEntry.
@param position The list position of the entry to replace.
@param newEntry The replacement entry.
@return boolean indicating if the operation was successful */
  virtual bool replace(int position, const ItemType & newEntry) = 0;

/**Swaps the entries of two different positions in the list.
@pre none
@post The entries will now have swapped positions in the list
@param positionA the first entry at positionA to be swapped
@param positionB the second entry at positionB to be swapped with positionA
@return boolean indicating if the operation was successful */
  virtual bool swap(int positionA, int positionB) = 0;
