...
//private
template<class ItemType>
ItemType Bag<ItemType>::getItem(int index) const
{
    if(index >= DEFAULT_BAG_SIZE || index < 0)
        return NULL;
    return items[index];
}   // end of getItem

template<class ItemType>
Bag Bag<ItemType>::bagUnion(in bag : Bag)
{
    ItemType[] newItems = new ItemType[itemCount + bag.getCurrentSize()];

    for(int i = 0; i < itemCount; i++)
    {
        for(int j = 0; j < bag.getCurrentSize)
    }
}
// private
template<class ItemType>
